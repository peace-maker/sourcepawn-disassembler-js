...
import { finishedLoading } from "./sourcepawnfile";
...
export default class FileHeader {
    ...
    public constructor(data: ArrayBuffer, callback: finishedLoading) {
        ...
        switch (this.compression) {
            ...
            case CompressionType.Gz:
                ...
                zlib.inflateRaw(compressed, (err: Error | null, result: Buffer) => {
                    if (err != null) {
                        throw err;
                    }

                    result.copy(array, this.dataoffs);
                    this.readSections(callback);
                });
                break;
            default:
    ...
    private readSections(callback: finishedLoading): void {
